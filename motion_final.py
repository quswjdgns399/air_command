...
class AppUI(QWidget):
    ...
    def train_model(self):
        ...
        try:
            ...
            acc_ax.plot(history.history['val_acc'], 'g', label='vapyinstaller --onefile your_script.pyl acc')
    ...
    def unlock_password(self):
        ...
        def open_file():
            subprocess.Popen(["python", "dd.py"])  # 여기에 실제 Python 파일 이름을 입력하세요
            sys.exit()
        ...
        while cap.isOpened():
            ...
            if result.multi_hand_landmarks is not None:
                for res in result.multi_hand_landmarks:
                    ...
                    if this_action == 'ac1' and action_counter.get(this_action, 0) >= count_threshold:
                        ac1_count += 10
                        if ac1_count == 10:
                            ...
                            cv2.imshow('img', img)
                        action_counter = {}
                    if this_action == 'ac2' and action_counter.get(this_action, 0) >= count_threshold:
                        ac2_count += 10
                        if ac2_count == 10:
                            cv2.putText(img, f'Waiting for next action...', org=(10, 30),
                                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255),
                            ...
                            cv2.imshow('img', img)
                        action_counter = {}
                    if this_action == 'ac3' and action_counter.get(this_action, 0) >= count_threshold:
                        ac3_count += 10
                        if ac3_count == 10:
                            cv2.putText(img, f'Waiting for next action...', org=(10, 30),
                                        fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1, color=(255, 255, 255),
                    ...
                    if sum_of_count >= 30:
                        show_notification("모션인식 암호가 설정 되었습니다.")
                        sys.exit()
